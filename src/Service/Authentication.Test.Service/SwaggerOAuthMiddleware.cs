...
namespace Authentication.Test.Service
{
    public class SwaggerOAuthMiddleware
    {
        ...
        {
            if(IsSwaggerUi(context.Request.Path) && !await IsValidToken(GetIdTokenFromSessionStorage(context)))
            {
        ...
        {
            var swaggerOpenIdSignInCallBack = _configuration.GetValue<string>("AzureAd:CallbackPath");
        ...
        private static async Task<string> GetIdTokenFromSignInCallBackRequest(HttpContext context)
        {
            ...
            return body.Split("&").FirstOrDefault(e => e.Contains("id_token"))?
                .Split("=").LastOrDefault();
        }

        private async Task<bool> IsValidToken(string idToken)
        {
            ...
            var swaggerUiGetwayClientId = _configuration.GetValue<string>("AzureAd:ClientId");
            var jwtHandler = new JwtSecurityTokenHandler();
            var result = await jwtHandler.ValidateTokenAsync(idToken,
                new TokenValidationParameters
                {
                    ValidateAudience = true,
                    ValidAudience = swaggerUiGetwayClientId,
                    ValidateIssuer = true,
        ...
        private async Task LoadOpenIdConnectConfiguration()
        {
            if(_openIdConnectConfiguration is null)
            {
                ...
                var configManager = new ConfigurationManager<OpenIdConnectConfiguration>($"{instance}{tenantId}/v2.0/.well-known/openid-configuration",
